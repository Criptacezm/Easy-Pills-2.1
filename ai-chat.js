...
// 1. API Configuration - Key injected at build time or runtime
// For Vercel: Set GEMINI_API_KEY in Environment Variables
// The key will be replaced during build or fetched from a secure endpoint
const GEMINI_API_KEY = typeof window.__GEMINI_API_KEY__ !== 'undefined' 
    ? window.__GEMINI_API_KEY__ 
    : ''; // Will be empty if not configured - shows error message
...
async function callGeminiAPI(userPrompt) {
    ...
    if (!GEMINI_API_KEY) {
        return getCurrentLang() === 'ar'
            ? "⚠️ مفتاح API غير مُهيأ. يرجى إعداد متغير البيئة GEMINI_API_KEY."
            : "⚠️ API key not configured. Please set the GEMINI_API_KEY environment variable.";
    }
